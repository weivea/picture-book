...
export function assignVoices(
  ...
  // 跨池去重：主角/配角共享一个 used 集合，避免主角占用的 voice 又被配角抓走
  const usedVoices = new Set<string>();

  // 排序保证确定性：先主角，再按角色名字典序（显式 locale 避免运行时差异）
  ...
  for (const c of sorted) {
    ...
    let pick = parsed.supporting.find(
      (v) => v.gender === c.gender_hint && !usedVoices.has(v.id),
    );
    // 池耗尽兜底：允许跨性别，最后允许复用
    pick ??= parsed.supporting.find((v) => !usedVoices.has(v.id));
