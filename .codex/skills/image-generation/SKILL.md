...
description: |
  ...
  to PNG". One invocation = exactly one image; batching and retries are the
  caller's responsibility. Requires AZURE_API_KEY env var.
---
...
5. 退出码非零 → 把 stderr 返回给调用方，picture-book-creator 阶段 6.3 会处理失败页
...
- **忘记 `AZURE_API_KEY`**：脚本会立即报错；推荐在项目根目录 `.env` 中配置，或在启动 Codex 前把环境变量导出到父 shell
